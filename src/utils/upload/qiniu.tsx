 ...
 * @LastEditTime: 2021-06-17 19:17:23
...
const UploadText: React.FC<UploadTextProps> = (props) => {
  ...
  return (
    <>
      ...
      <p className="ant-upload-hint">请点击上传或者将文件拖到区域内上传</p>
    </>
  );
};
...
const UploadAntd: React.FC<UploadAntdProps> = (props) => {
  ...
  const storageType = localStorage.getItem(fileStorage || '')
    ? [...JSON.parse(String(localStorage.getItem(fileStorage || '')))]
    : ''; // 走缓存用这里
  ...
  const [fileList, setFileList] = useState<any>(
    judgeNullForUpload(storageType, propsType, propsFileItem),
  );
      ...
      return (
        <div onClick={getQiNiuToken}>
          <section style={{ width: dragSize, marginBottom: '30px' }}>
                ...
                return (
                  <>
                    ...
                    <div
                      ...
                      className="render_upload_list"
                    >
                      {childFileType === 'ppt' && file.url ? (
                        <Space>
                          <span>{file.url}</span>
                          <a download={file.url} href={file.url}>
                            点击下载
                          </a>
                          <IconFont
                            type="icon-icon--shanchu"
                            onClick={() => onRemoveItem(file)}
                            className="render_upload_delete_icon"
                          />{' '}
                        </Space>
                      ) : null}
            ...
            {fileList.length !== 0 ? (
              <div> {!fileList[0].url ? '文件未上传' : '文件已上传'} </div>
            ) : null}
          </section>
        </div>
      );
...
export { QINIU_SERVER, BASE_QINIU_URL, UploadText, UploadAntd, fileType, pictureSize };
